...
func (ns *NetworkServer) Set(ctx context.Context, req *ttnpb.SetEndDeviceRequest) (*ttnpb.EndDevice, error) {
	...
	// NOTE: Version-specific session contents are validated lower.
	...
	if err := st.ValidateIfNotZeroThenZero("supports_join",
		"multicast",
	); err != nil {
		return nil, err
	}
	...
	if err := st.ValidateIfZeroThenZero("session.dev_addr",
		"multicast",
		// TODO
	); err != nil {
		return nil, err
